...
| **Dependencies** | Python stdlib + SQLite | — | — | varies |
...
- **Authentication + RBAC** — SQLite logins (PBKDF2-SHA256), server-side revocable sessions; roles:
  `superadmin` / `admin` (operate assigned tenants) / `viewer` (read assigned).
...
```
The dashboard binds `127.0.0.1:8807` — put a TLS reverse proxy in front for remote access and set
`PXTUNNEL_COOKIE_SECURE=1`. Then **Add device** from the UI and run the generated command on the machine.
...
- [ ] Devices model UI — list + card views, device detail, "system" infra section
